...
export const createTokensBottomBox = (project: Project) => {
    const tokensBottomBox = createBoxComponent(project, {
        name: "TokensBottom Box",
        materialThickness: 3,
        params: {
    ...
    createBottom(tokensBottomBox, {
        grid: createGrid({ width: 2 }, tokensBottomBox),
    });
    createLeftWall(tokensBottomBox);
    createRightWall(tokensBottomBox);
    createFrontWall(tokensBottomBox, {});
    createBackWall(tokensBottomBox, {});
    createDividers(tokensBottomBox);

    return tokensBottomBox;
};
